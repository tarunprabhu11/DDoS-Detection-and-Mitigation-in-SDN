    ...
    def flow_training(self):
        ...
        class FlowModel(nn.Module):
             ...
             def forward(self, x):
                 ...
                 return x
        X_flow_train_torch = torch.tensor(X_flow_train1, dtype=torch.float32).to(device)
        y_flow_train_torch = torch.tensor(y_flow_train, dtype=torch.float32).to(device)
        X_flow_test_torch = torch.tensor(X_flow_test1, dtype=torch.float32).to(device)

# Training
        input_size = X_flow_train_torch.shape[1]
        self.flow_model = FlowModel(input_size).to(device)
...
# Evaluation
    ...
    def flow_predict(self):
        try:
            ...
            X_predict_flow_torch = torch.tensor(X_predict_flow, dtype=torch.float32).to(device)
            self.logger.info("loaded")
            with torch.no_grad():
               self.flow_model.eval()
               y_flow_pred_torch = self.flow_model(X_predict_flow_torch)
            threshold = 0.5
            binary_predictions = torch.where(y_flow_pred_torch > threshold, 1, 0).flatten().to("CPU").numpy()
